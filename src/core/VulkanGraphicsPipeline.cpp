...
VulkanGraphicsPipeline::VulkanGraphicsPipeline(VulkanGraphicsPipelineCreateInfo* info) {
    VkShaderModule vertShaderModule = createShaderModule(info->device, info->vertexShaderCode);
    VkShaderModule fragShaderModule = createShaderModule(info->device, info->fragmentShaderCode);
    ...
    if (vkCreatePipelineLayout(info->device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        spdlog::info("Failed to create graphics pipeline layout!");
        throw 0;
    }
    ...
    pipelineInfo.subpass = info->numSubPasses;
